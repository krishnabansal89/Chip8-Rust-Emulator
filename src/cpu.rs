...
impl Cpu{
    ...
    fn run_instrutions(&mut self){
        let opcode = self.fetch_opcode();
        self.execute_opcode(opcode);
    }

    fn fetch_opcode(&self) -> u16{
        let opcode = (self.ram.read_byte(self.pc) as u16) << 8 | self.ram.read_byte(self.pc + 1) as u16;
        self.pc += 2;
        opcode
    }
    ...
    fn execute_opcode(&mut self, opcode : u16){
        ...
        match opcode & 0xF000{
            0x0000 => {
                match opcode & 0x00FF{
                    0xE0 => self.clear_screen(), // to do later
                    0xEE => {
